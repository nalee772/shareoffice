		...
		<div class="madmin_container" id="wrap">
					...
					<c:if test="${totCnt > 0 }">
						<c:forEach var="member" items="${list }">
							<tr>
								<td>${startNum }</td>
								<td><a
									href="memInfo.do?mbr_id=${member.mbr_id}&pageNum=${currentPage}&admin_mbr_id=${user.mbr_id}">
										${member.mbr_name } </a></td>
								<td>${member.mbr_email }</td>
								<td><c:if test="${member.mbr_status ==0}">탈퇴</c:if> <c:if
										test="${member.mbr_status ==2}">정지</c:if> <c:if
										test="${member.mbr_status ==3}">추방</c:if></td>
							</tr>
			...
			<div class="madmin_footer">
				<div class="madmin_footer_search">
					...
					<form action="memSearch.do?pageNum=${pageNum}" name="frm"
						onsubmit="return chk()">
						<!-- 경로 -->
						<select class="msearch_select" name="msearch">
							<option name="mbr_name" value="mbr_name">회원명</option>
							<option name="mbr_id" value="mbr_id">회원 아이디</option>
							<option name="id_name" value="id_name">회원명 + 아이디</option>
						</select> <input type="text" class="msearch_bar" name="memSearch"
							id="memSearch" required="required"> <input type="submit"
							class="btn btn-dark" value="검색"
							onclick="location.href='memSearch.do?pageNum=${pageNum }'">
					</form>
				</div>
				<div class="page">
					<div class="page_container">
						<c:if test="${startPage > blockSize }">
							<div class="page_btn">
								<a href='memList.do?pageNum=${startPage-blockSize }'>
