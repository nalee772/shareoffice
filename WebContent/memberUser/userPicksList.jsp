...
<head>
<title>Share Office | 회원 찜목록</title>
...
<input type="hidden" name="ofc_code" value="${picks.ofc_code }"> 
<input type="hidden" name="ofctype_code" value="${picks.ofctype_code}"> 
	<main class="userPicksList__main"> <!-- 조건 검색 결과--> 
	<c:forEach items="${userPicksList}" var="picks">
		<!-- SearchForm에서 선택한 오피스의 정보 -->
		<section class="userPicksList__main-section">
	<!-- 		<div class="userBooksList__main-section-list"> -->
				<!-- 검색 조건에 맞는 오피스 목록 리스트 -->
				<div class="userPicksList__main-section-list-article userPicksList-list">
					<div class="userPicksList-list-image">
						<!-- 오피스 대표 섬네일 사진 -->
						<img src="${picks.picture1 }" alt="userPicksList-list-image" />
					</div>
					<div class="userPicksList-list-text">
						<!-- 오피스 이름 -->
						<h1 class="userPicksList-list-text-header">${picks.ofc_name }</h1>
						<h2 class="userPicksList-list-text-content">
							<!-- 예약 유형 -->
							<p>예약 유형 : ${picks.ofctype_name }</p>
							<!-- 예약 유형 -->
							<p>주소 : ${picks.ofc_location }</p>
							<!-- 예약 시작 -->
							<p>번호 : ${picks.ofc_tel }</p>
							<form action="userPicksDelete.do" method="post">
								<input type="hidden" name="mbr_id" value="${user.mbr_id}">
								<input type="hidden" name="pageNum" value="${pageNum}">
								<input type="hidden" name="ofc_code" value="${picks.ofc_code }"> 
								<input type="hidden" name="ofctype_code" value="${picks.ofctype_code}"> 
								<input class="type-btn" type="submit" value="찜취소" />
							</form>
						</h2>
					</div>
				</div>
